...
public class jaskierKarczmaFightBoss {
    @FXML
    ...
    public jaskierKarczmaFightBoss() throws IOException {
    }

    @FXML
    protected void initialize() {
        banditHP.setText(String.valueOf(banditboss.maxHP));
        banditPower.setText(String.valueOf(banditboss.power));
        geraltPower.setText(String.valueOf(geralt.power));
        geraltHP.setText(String.valueOf(geralt.maxHP));
        banditRemainingHP.setText(String.valueOf(banditboss.currentHP));
    ...
    protected void onNormalAttack(ActionEvent actionEvent) throws IOException {
        ...
        if (geralt.currentHP - receivedDMG1 <= 0) {
            banditboss.currentHP = banditboss.maxHP;
            geralt.death(actionEvent);
        }
        if (banditboss.currentHP - dealtDMGn <= 0) {
            win(actionEvent);
        }

        if (geralt.currentHP > 0 && banditboss.currentHP > 0) {
            textDealtDamage.setText("Zadane obrażenia:");
            dealtDamage.setText(String.valueOf(dealtDMGn));
            banditboss.currentHP -= dealtDMGn;
    ...
    private int receivedDMG1(ActionEvent actionEvent) throws IOException {
        int receivedDMG1 = (int) banditboss.onAttack(actionEvent);
        return receivedDMG1;
    }
    ...
    protected void onHardAttack(ActionEvent actionEvent) throws IOException {
        ...
        if (banditboss.currentHP - dealtDMGS <= 0) {
            win(actionEvent);
        }
        ...
        if (geralt.currentHP > 0 && banditboss.currentHP > 0) {
            ...
            banditboss.currentHP -= dealtDMGS;
    ...
    private void win(ActionEvent actionEvent) throws IOException {
        geralt.money += (500 + jaskierKarczma.banditMoney);
        geralt.moc += 10;
        geralt.maxHP += 10;
        geralt.power = ((10 + 20) / 2) + geralt.moc;
        FXMLLoader fxmlLoader = new FXMLLoader(start.class.getResource("jaskierKarczma/win4.fxml"));
